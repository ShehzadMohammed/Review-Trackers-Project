...
(Get-Content ./Terraform-Infrastructure/variables.tf).Replace('replace-variable', $paramforstr) | Set-Content ./Terraform-Infrastructure/variables.tf #This sets the variable to the input given
...
$date = (Get-Content ./Terraform-Infrastructure//don.txt).ForEach({ '"{0}"' -f $_ })

# $SAS = (Get-Content ./SAStoken.txt).ForEach({ '"{0}"' -f $_ })
$SAN = (Get-Content ./Terraform-Infrastructure//SAN.txt).ForEach({ '"{0}"' -f $_ })
$SCA = (Get-Content ./Terraform-Infrastructure//SCA.txt).ForEach({ '"{0}"' -f $_ })
$PAK = (Get-Content ./Terraform-Infrastructure//PAK.txt).ForEach({ '"{0}"' -f $_ })
$key = "terraform.tfstate"
#Formats and initializes the variables used in the migration process from local to azurerm 


(Get-Content ./Terraform-Infrastructure/versions.tf).Replace('"local"', '"azurerm"') | Set-Content ./Terraform-Infrastructure/versions.tf
...
(Get-Content ./Terraform-Infrastructure/variables.tf).Replace($storageName, 'replace-variable') | Set-Content ./Terraform-Infrastructure/variables.tf
(Get-Content ./Terraform-Infrastructure/versions.tf).Replace('"azurerm"', '"local"') | Set-Content ./Terraform-Infrastructure/versions.tf
