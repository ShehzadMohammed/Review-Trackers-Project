...
terraform init -migrate-state -backend-config="storage_account_name=$SAN" -backend-config="access_key=$ARM_ACCESS_KEY" -backend-config="use_msi=false" -backend-config="use_microsoft_graph=false" -backend-config="container_name=$SCA" -backend-config="key=$KEY" --auto-approve
